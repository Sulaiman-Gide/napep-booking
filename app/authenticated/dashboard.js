...
export default function DriverDashboard() {
  ...
  const acceptTrip = (trip) => {
    ...
    Alert.alert('Trip Accepted', `Pickup at: ${trip.pickup.latitude}, ${trip.pickup.longitude}`);
  };
  ...
  const startNavigation = () => {
    ...
    const distance = getDistance(
      ...
      { latitude: selectedTrip.pickup.latitude, longitude: selectedTrip.pickup.longitude }
    );

    Alert.alert('Navigation Started', `Distance to passenger: ${distance / 1000} km`);
  };
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContainer}>
        ...
        {availableTrips.length > 0 ? (
          availableTrips.map((trip, index) => (
            ...
            >
              <Text style={styles.tripText}>Pickup: {trip.pickup.latitude}, {trip.pickup.longitude}</Text>
              <Text style={styles.tripText}>Destination: {trip.destination.latitude}, {trip.destination.longitude}</Text>
              <Text style={styles.tripCost}>Cost: ₦{trip.cost ? trip.cost.toFixed(2) : 'N/A'}</Text>
            </TouchableOpacity>
          ))
        ) : (
        ...
        {selectedTrip && (
          <>
            <View style={styles.mapContainer}>
              ...
              >
                <Marker coordinate={selectedTrip.pickup} title="Pickup Location" />
                <Marker coordinate={selectedTrip.destination} title="Destination" />
              </MapView>
            </View>

            <TouchableOpacity onPress={startNavigation} style={styles.navigateButton}>
